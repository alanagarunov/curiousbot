...
def getstockprice():
    ...
    conn.close()
    thethemessage = "The current price is " + str(initialprice) + " and the current available volume is " + str(totalvolume)
...
def adddata(named, sharetype, amount):
    ...
    d.execute("INSERT INTO stock_values VALUES (:shareprice, :volume)", {'shareprice': 5.1264, 'volume': 10000})
    ...
    if not row: #and type(row) is not list
        c.execute("INSERT INTO portfolio VALUES (:name, :sharesowned, :value)", {'name': str(named), 'sharesowned': 0, 'value': 100 })
        ...
        if float(newrow[2]) < purchase:
        ...
        elif int(newrow[1]) > totalvolume or int(amount) < 0 or (totalvolume - int(amount)) < 0 or int(amount) > (float(totalvolume)*0.5):
            ...
            c.execute("UPDATE portfolio SET sharesowned = :sharesowned WHERE name = :name", {'name': str(named), 'sharesowned': int(tempamount)})
            c.execute("UPDATE portfolio SET value = :value WHERE name = :name", {'name': str(named), 'value': float(temppurchase)})
            ...
            d.execute("UPDATE stock_values SET shareprice = :shareprice", {'shareprice': float(initialprice)})
            d.execute("UPDATE stock_values SET volume = :volume", {'volume': int(totalvolume)})
            '''
            thetime = round(time.time())
            price.append(initialprice)
            timesincestart.append(thetime)
            '''
            sucessfulmessage = "<@!" + str(named) + ">" + " has bought " + str(amount) + " of share(s) " + "for a total of " + str(purchase) + ". The stock is now worth " + str(initialprice)
    elif sharetype == 'sell':
        newrow = row[0]
        purchase = initialprice * float(amount)
        purchase = round(purchase, 2)
        temppurchase = float(newrow[2]) + float(purchase)
        temppurchase = round(temppurchase, 2)
        if newrow[1] < int(amount) or int(amount) < 0 or (totalvolume - int(amount)) < 0:
            ...
            timesincestart.append(thetime)
            '''
            sucessfulmessage = "<@!" + str(named) + ">" + " has sold " + str(amount) + " of share(s) " + "for a total of " + str(purchase) + ". The stock is now worth " + str(initialprice)
            
    else:
        sucessfulmessage = "Either you have more than one portfolio entry or something went wrong."
    conn.commit()
    conn1.commit()
